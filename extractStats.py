...
def getStatFile(tempFile = TEMP_STAT_FILE):
    os.system("git log --shortstat --all --date=short > {}".format("./__out__"))
    stats = open(tempFile)
    return stats
...
parser.add_argument("--begin", nargs = 3, type = int,
                    ...
                    help = "Date de debut de la periode a couvrir")
parser.add_argument("--end", nargs = 3, type = int,
                    metavar=("ANNEE","MOIS","JOUR"),
                    help="Date de fin de la periode a couvrir")
...
parser.add_argument("--tasks", nargs = "*", metavar = "TACHE",
                    help = "La liste des taches pour lesquelles on veut extraire les statistiques. Les"
